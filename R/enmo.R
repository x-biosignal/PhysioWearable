...
#' magnitude about its (per-epoch) mean (Vaha-Ypya et al. 2015): for each epoch,
#' \eqn{{\rm MAD} = \frac{1}{n}\sum_i |r_i - \bar r|}, where
#' \eqn{r_i = \sqrt{x_i^2 + y_i^2 + z_i^2}} is the sample vector magnitude and
#' \eqn{\bar r} its epoch mean. It is a gravity-robust movement-intensity metric
#' complementary to \code{\link{computeENMO}}: MAD captures the amount of
#' \emph{variation} in the magnitude within an epoch (so it is insensitive to a
#' static orientation offset), whereas ENMO captures its \emph{elevation} above
#' 1 g. When an epoch length is given, per-epoch MAD is returned; otherwise a
#' single whole-signal MAD.
#'
#' @param accel An n x 3 matrix (or data frame) of acceleration in g units
#'   (columns x, y, z).
#' @param sampling_rate Sampling rate in Hz (required if `epoch_sec` is set).
#' @param epoch_sec Epoch length in seconds for aggregation; `NULL` returns a
#'   single MAD over the whole signal.
#' @param unit `"g"` (default) or `"mg"` (milli-g).
#'
#' @return A numeric vector of MAD values (one per epoch, or a single value).
#' @references Vaha-Ypya H, et al. (2015). Clin Physiol Funct Imaging 35(1):64-70.
#' @seealso [computeENMO()], [classifyBouts()]
#' @export
#' @examples
#' set.seed(1)
...
computeMAD <- function(accel, sampling_rate = NULL, epoch_sec = NULL,
                       unit = c("g", "mg")) {
  unit <- match.arg(unit)
  m <- .accel_matrix(accel)
  vm <- sqrt(rowSums(m^2))
  ...
  if (!is.null(epoch_sec)) {
    ...
    epoch_n <- max(1L, as.integer(round(epoch_sec * sampling_rate)))
    if (epoch_n > length(vm)) {
      stop(sprintf("epoch_sec (%g s) is longer than the signal.", epoch_sec),
           call. = FALSE)
    }
    n_ep <- length(vm) %/% epoch_n
    em <- matrix(vm[seq_len(n_ep * epoch_n)], nrow = epoch_n)   # columns = epochs
    mad <- colMeans(abs(sweep(em, 2L, colMeans(em), "-")))      # per-epoch mean abs deviation
  } else {
    mad <- mean(abs(vm - mean(vm)))
  }
  if (unit == "mg") mad <- mad * 1000
  mad
}

#' ENMO-abs (ENMOa) acceleration metric
#'
#' ENMOa is the "always-positive" companion of \code{\link{computeENMO}}: the
#' vector magnitude minus one gravity, with the \emph{absolute value} taken
#' instead of truncating negatives to zero, \eqn{{\rm ENMOa} = |r - 1|} with
#' \eqn{r = \sqrt{x^2+y^2+z^2}} (van Hees et al. 2013). It differs from ENMO only
#' where the magnitude dips below 1 g (deceleration / partial free-fall), which
#' ENMO truncates away; ENMOa retains that movement, so it is used when the
#' sensor calibration is imperfect (a persistent sub-1 g offset would otherwise
#' be lost by ENMO's truncation). When an epoch length is given, per-epoch mean
#' ENMOa is returned.
#'
#' @param accel An n x 3 matrix (or data frame) of acceleration in g units
#'   (columns x, y, z).
...
#' @param unit `"g"` (default) or `"mg"` (milli-g).
#'
#' @return A numeric vector of ENMOa values (per sample, or per epoch).
#' @references van Hees VT, et al. (2013). PLoS ONE 8(4):e61691.
#' @seealso [computeENMO()], [computeMAD()]
#' @export
#' @examples
#' set.seed(1)
#' acc <- matrix(rnorm(300, c(0, 0, 1), 0.1), ncol = 3, byrow = TRUE)
#' mean(computeENMOa(acc))          # >= mean(computeENMO): retains sub-1 g dips
computeENMOa <- function(accel, sampling_rate = NULL, epoch_sec = NULL,
                         unit = c("g", "mg")) {
  unit <- match.arg(unit)
  m <- .accel_matrix(accel)
  vm <- sqrt(rowSums(m^2))
  enmoa <- abs(vm - 1)                                # |r - 1| (vs ENMO's pmax(r - 1, 0))
  ...
  if (!is.null(epoch_sec)) {
    ...
    epoch_n <- max(1L, as.integer(round(epoch_sec * sampling_rate)))
    if (epoch_n > length(enmoa)) {
      stop(sprintf("epoch_sec (%g s) is longer than the signal.", epoch_sec),
           call. = FALSE)
    }
    enmoa <- .epoch_mean(enmoa, epoch_n)
  }
  if (unit == "mg") enmoa <- enmoa * 1000
  enmoa
}
